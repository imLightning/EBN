import torch
import numpy as np
from algos.SARL.policy import Policy
import itertools
from utils.state import ObservableState, FullState

class MultiHumanRL(Policy):
    def __init__(self):
        super().__init__()
    ...
    def build_action_space(self):
        ...
        """
        speeds = np.linspace(0.0, 0.4, self.speed_samples)
        rotations = np.linspace(-0.5, 0.5, self.rotation_samples)

        action_space = [np.zeros(2, dtype=np.float32)]
    ...
    def propagate(self, state, action):
        if isinstance(state, ObservableState):
            ...
            next_px = state.px + action[0] * self.time_step
            next_py = state.py + action[1] * self.time_step
            next_state = ObservableState(next_px, next_py, action[0], action[1], state.radius)
        elif isinstance(state, FullState):
            next_theta = state.theta + action[1] * self.time_step
            next_vx = action[0] * np.cos(next_theta)
            next_vy = action[0] * np.sin(next_theta)
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            for action in self.action_space:
                ...
                if self.query_env:
                    ...
                    next_human_states = [self.propagate(human_state, np.array([human_state.vx, human_state.vy], dtype=np.float32))
