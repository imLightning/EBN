...
from algos.TD3_LSTM.TD3 import TD3
from algos.TD3_LSTM.memory import ReplayBuffer, eval_policy_mode, set_seed_everywhere
from info import *
from env_lndnl import CrowdSim
...
def eval_policy(policy, eval_or_test_env, current_steps, 
    ...
    for i in range(eval_episodes):
        ...
        if final_test:
            lidar_state, robot_goal_state  = eval_or_test_env.reset(seed=i, save_data=if_save_data)
        else:
            lidar_state, robot_goal_state  = eval_or_test_env.reset(save_data=if_save_data)
        ...
        while ep_step < eval_or_test_env.max_episode_step:
            # t1 = time.time()
            with torch.no_grad():
                action, hidden = policy.select_action(lidar_state, robot_goal_state, hidden)
            # action = eval_or_test_env.dwa_compute_action()
            lidar_state, robot_goal_state, reward, done, info = eval_or_test_env.step(action, eval=True, save_data=if_save_data)
...
def main():
    ...
    parser.add_argument("--policy", default="td3_lstm")
    # options, differential, lip, digit_mujoco, now digit_arsim is not supported for training
    ...
    # if using emition related states
    parser.add_argument('--use_emotion', type=bool, default=False)
    # device
    ...
    parser.add_argument("--policy_noise", default=0.25, type=float)
    ...
    agent = TD3(
            ...
            args.lidar_feature_dim,
            args.action_dim,
            action_range,
            args.hidden_dim,
            discount=args.discount,
            tau=args.tau,
            policy_noise=args.policy_noise * (action_range[1] - action_range[0]),
            noise_clip=args.noise_clip * (action_range[1] - action_range[0]),
    ...
    lidar_state, robot_goal_state = env.reset()
    done = False
    episode_reward = 0
    episode_timesteps = 0
    episode_num = 0
    ...
    for t in range(checkpoint_steps + 1, int(args.max_timesteps) + 1):
        ...
        if t < args.start_timesteps:
            ...
            with torch.no_grad():
                _, next_hidden = agent.select_action(lidar_state, robot_goal_state, hidden)
        else:
            with torch.no_grad():
                a, next_hidden = agent.select_action(lidar_state, robot_goal_state, hidden)
            action = (
                a + np.random.normal(np.zeros(args.action_dim), (action_range[1] - action_range[0]) * args.expl_noise)
            ).clip(action_range[0], action_range[1])
        next_lidar_state, next_robot_goal_state, reward, done, info = env.step(action)
        ...
        replay_buffer.add(
            lidar_state, robot_goal_state, action, next_lidar_state, next_robot_goal_state, reward, done_bool, hidden, next_hidden)
        ...
        if t >= args.start_timesteps:
            agent.train(replay_buffer, writer, batch_size=args.batch_size)
        ...
        if t % args.eval_freq == 0:
            ...
            if success_rate > 0.85 or t % args.save_model_freq == 0 or t <= 300000:
